...
def build_config(config):
    ...
    Prec=config["Architecture"]["Prec"]
    shot=config["dataset"]["target_shot"]
    ...
    if config["warmup"]:
        ...
        config["output_path_warmup"] = os.path.join(
            ...
            config_data["name"],
            f"{source_name}_to_{target_name}_{method}_{shot}_pretrained_warmup",
        )
