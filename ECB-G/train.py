...
def parse_opt(source,target,known=False):
    parser = argparse.ArgumentParser(description="The proposed method")
    parser.add_argument(
        "--cfg",
        type=str,
        default="configs/DomainNet/Gate/train_{}_{}.yaml".format(source,target),
...
def main(args):
    ...
    if config["Need_pretrained"] == True:
        G1, G2, F1, F2 = build_model(config_architecture, DEVICE=config["DEVICE"], pretrain=True)
    else:
        G1, G2, F1, F2 = build_model(config_architecture, DEVICE=config["DEVICE"], pretrain=False)
    #################
...
if __name__ == "__main__":
    args = parse_opt(source='P', target='R')
    main(args)
