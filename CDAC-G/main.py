...
from model.basenet import AlexNetBase, Predictor, Predictor_deep, Predictor_deep_gate, Predictor_gate
...
parser.add_argument('--method', type=str, default='CDAC_Gate', choices=['CDAC', 'CDAC_Gate'],
                    help='CDAC is proposed method')
...
parser.add_argument('--source', type=str, default='painting', help='source domain')
...
def folder_preparation(args):
    ...
    main_path = 'record_4090/%s/%s_%d' % (args.dataset, args.method, args.num)
    # logs saving
    logs_file = os.path.join(main_path, 'net_%s_%s_to_%s_num_%s_rampup_%d_logs_test.txt' % (args.net, args.source,
                                                                                       args.target, args.num,
                                                                                       args.rampup_length))

    # checkpath saving
    checkpath = os.path.join(main_path, 'checkpath/net_%s_%s_to_%s_num_%s_new_/' % (args.net, args.source,
                                                                               args.target, args.num))
    ...
    if value.requires_grad:
        if 'classifier' not in key:
            ...
            params += [{'params': [value], 'lr': args.multi * 10,
                        'weight_decay': 0.0005}]
if args.method == 'CDAC_Gate':
    F1 = Predictor_gate(num_class=len(class_list), inc=inc, temp=args.T)
else:
...
source_loader = torch.utils.data.DataLoader(source_dataset, batch_size=bs, num_workers=4, shuffle=True, pin_memory=True,
                                            drop_last=True)
target_loader = torch.utils.data.DataLoader(target_dataset, batch_size=min(bs, len(target_dataset)), num_workers=4,
                                            shuffle=True, pin_memory=True, drop_last=True)
target_loader_unl = torch.utils.data.DataLoader(target_dataset_unl, batch_size=bs * 2, num_workers=4, shuffle=True,
...
target_loader_test = torch.utils.data.DataLoader(target_dataset_test, batch_size=bs * 2, num_workers=4, shuffle=True,
                                                 pin_memory=True, drop_last=True)
...
def train(device, opt):
    with open(opt["logs_file"], 'a') as f:
        f.write(
            '==================This is Run for {} to {}, method {}, steps: {}  lr :{} rampup_length {} ,  C  ===============\n'.format(
                args.source, args.target, args.method, args.steps, args.lr, args.rampup_length))
    G.train()
    F1.train()
    ...
    best_acc_test, best_acc = 0, 0

    BCE = BCE_softlabels().to(device)
    criterion = nn.CrossEntropyLoss().to(device)

    with open(opt["logs_file"], 'a') as f:
        f.write('This is test with steps {}'.format(args.steps))
        ...
        if args.save_check:
            if step%10000==9999:
                torch.save(G.state_dict(), os.path.join(opt["checkpath"],
                                                        "G_iter_model_{}_to_{}_in_{}_step_{}.pth.tar".format(args.source,
                                                                                                     args.target,
                                                                                                     args.method,step
                                                                                                     )))
                torch.save(F1.state_dict(), os.path.join(opt["checkpath"],
                                                         "F1_iter_model_{}_to_{}_in_{}_step_{}.pth.tar".format(args.source,
                                                                                                       args.target,
                                                                                                       args.method,step
                                                                                                       )))
    if args.save_check:
        print('Saving model')
        torch.save(G.state_dict(), os.path.join(opt["checkpath"],
                                                "G_iter_model_{}_to_{}_in_{}.pth.tar".format(args.source,
                                                                                             args.target, args.method
                                                                                             )))
        torch.save(F1.state_dict(), os.path.join(opt["checkpath"],
                                                 "F1_iter_model_{}_to_{}_in_{}.pth.tar".format(args.source,
                                                                                               args.target, args.method
                                                                                               )))
