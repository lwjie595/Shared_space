...
def return_dataset(args):
    ...
    image_set_file_t_val = \
        os.path.join(base_path,
                     ...
                     args.target + '_3.txt')
    image_set_file_s_val=os.path.join(base_path,'validation_target_images_'+args.source+'_3.txt')
    image_set_file_unl = \
        os.path.join(base_path,
                     'unlabeled_target_images_' +
                     args.target + '_%d.txt' % (args.num))
    image_set_file_t_test= \
        os.path.join(base_path,'labeled_source_images_' +
                     args.target + '.txt')
    if args.net == 'alexnet':
        crop_size = 227
    else:
    ...
    ###target_dataset_test之前用过image_set_file_t_test 作为测试集
    ...
    source_loader = torch.utils.data.DataLoader(source_dataset, batch_size=bs,
                                                ...
                                                drop_last=True)
    # source_loader2=torch.utils.data.DataLoader(source_dataset2,batch_size=bs,num_workers=4,shuffle=True,pin_memory=True,drop_last=True)
    source_loader2=0
    target_loader = \
    ...
    target_loader_test = \
        torch.utils.data.DataLoader(target_dataset_test,
                                    batch_size=bs * 2, num_workers=4,
                                    shuffle=False,pin_memory=True, drop_last=True)
    return source_loader, source_loader2,target_loader, target_loader_unl,\
        target_loader_val,source_loader_val, target_loader_test, class_list
